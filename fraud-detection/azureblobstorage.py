...
def upload_file(container_name, bytes_data, file_name, content_type='application/pdf'):
    ...
    return blob_client.url + '?' + generate_blob_sas(account_name, container_name, file_name,account_key=account_key,  permission="r", expiry=datetime.utcnow() + timedelta(days=365))
...
def get_all_files():
    ...
    sas = generate_container_sas(account_name, container_name,account_key=account_key,  permission="r", expiry=datetime.utcnow() + timedelta(year=1))
    files = []
    converted_files = {}
...
def get_all_images(container_name):
    # Get all images in the container from Azure Blob Storage
    account_name = os.environ['BLOB_ACCOUNT_NAME']
    account_key = os.environ['BLOB_ACCOUNT_KEY']
    connect_str = f"DefaultEndpointsProtocol=https;AccountName={account_name};AccountKey={account_key};EndpointSuffix=core.windows.net"
    # container_name = os.environ['BLOB_CONTAINER_NAME']
